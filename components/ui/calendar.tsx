...
function CalendarHeader() {
  ...
  return (
    <header className="flex w-full items-center gap-1 px-1 pb-4">
      <Button size="square-petite" appearance="outline" slot="previous">
        {direction === 'rtl' ? (
      ...
      <Button size="square-petite" appearance="outline" slot="next">
        {direction === 'rtl' ? (
          <IconChevronLgLeft aria-hidden />
        ) : (
          <IconChevronRight aria-hidden />
        )}
      </Button>
      ...
      <CalendarGrid className="[&_td]:px-0">
        ...
        <CalendarGridBody>
          {(date) => (
            <CalendarCell
              date={date}
              className="group size-9 cursor-default text-sm outline outline-0 outside-month:text-gray-300 selected:bg-blue-100 invalid:selected:bg-red-100 selection-start:rounded-s-full selection-end:rounded-e-full dark:selected:bg-blue-700/30 dark:invalid:selected:bg-red-700/30 forced-colors:selected:bg-[Highlight] forced-colors:invalid:selected:bg-[Mark] [td:first-child_&]:rounded-s-full [td:last-child_&]:rounded-e-full"
            >
              {({
                formattedDate,
                isSelected,
