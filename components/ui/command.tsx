    ...
    <ModalOverlay {...props}>
      ...
      >
        <Dialog aria-label="Command Palette">
          {({ close }) => (
            <>
              <Command className="[&_[cmdk-group-heading]]:ml-1 [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group-heading]]:text-muted-fg [&_[cmdk-group]:not([hidden])_~[cmdk-group]]:pt-0 [&_[cmdk-group]]:px-0 [&_[cmdk-input-wrapper]_svg]:size-5 [&_[cmdk-input]]:h-12 [&_[cmdk-item]]:py-[0.70rem] [&_[cmdk-item]]:pl-3 [&_[cmdk-item]]:pr-4 [&_[cmdk-item]_svg]:mr-2 [&_[cmdk-item]_svg]:size-[1.10rem]">
                {children}
              </Command>
              <Button
                autoFocus={!isDesktop}
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ref={ref}
    className={cn(
      'relative flex cursor-default select-none items-center rounded-lg px-2 py-1.5 text-sm outline-none data-[disabled=true]:pointer-events-none data-[selected=true]:bg-accent data-[selected=true]:text-accent-fg data-[disabled=true]:opacity-50 [&[data-selected=true]_svg]:text-accent-fg [&_svg]:shrink-0 [&_svg]:text-muted-fg',
      className
    )}
    {...props}
  />
